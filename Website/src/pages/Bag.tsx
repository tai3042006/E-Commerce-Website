  ...
  return (
    <Layout>
      <section className="container-clofit pt-4 lg:pt-10">
        <h1 className="text-2xl font-extrabold tracking-tight lg:text-4xl">Bag ({count})</h1>
        ...
        {detailed.length === 0 ? (
          <div className="py-24 text-center">
            ...
            <h2 className="mt-5 text-lg font-semibold">Your bag is empty</h2>
            <p className="mt-1 text-sm text-muted-foreground">Discover something you'll love.</p>
            <Link to="/shop" className="mt-6 inline-block rounded-pill bg-foreground px-6 py-3 text-sm font-semibold text-background">
              Shop the collection
            </Link>
          </div>
        ) : (
            ...
            <div className="space-y-1">
              <AnimatePresence>
                {detailed.map(({ item, product }) => (
                  <motion.div key={product.id + item.size} layout
                    ...
                    <Link to={`/product/${product.id}`} className="h-28 w-24 flex-shrink-0 overflow-hidden rounded-xl bg-secondary sm:h-36 sm:w-32">
                    ...
                    <div className="flex flex-1 flex-col">
                      <div className="flex justify-between gap-2">
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-muted-foreground sm:text-sm">{product.tagline}</p>
                          <p className="mt-0.5 text-xs text-muted-foreground sm:text-sm">Size: {item.size}</p>
                        </div>
                        <p className="text-sm font-semibold sm:text-base">${(product.price * item.qty).toFixed(2)}</p>
                      </div>
                      <div className="mt-auto flex items-center justify-between gap-3 pt-3">
                        ...
                        <button onClick={() => remove(item.id, item.size)} aria-label="Remove"
            ...
            <aside className="lg:sticky lg:top-28 lg:self-start">
              <div className="rounded-2xl border border-border p-6">
                ...
                <div className="mt-5 flex gap-2">
                  ...
                  <button onClick={applyPromo} className="rounded-pill border border-border px-4 py-2.5 text-sm font-medium hover:bg-secondary">
                    Apply
                  </button>
                </div>
                ...
                <dl className="mt-5 space-y-3 text-sm">
                  ...
                  {discount > 0 && <div className="flex justify-between text-[hsl(var(--success))]"><dt>Discount</dt><dd>-${discount.toFixed(2)}</dd></div>}
